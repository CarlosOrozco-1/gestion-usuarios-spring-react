...
public interface CredentialRepository extends JpaRepository<Credential, Integer> {
    ...
    @Query("select new com.gestionusuarios.dto.stats.ClientCredentialCount(c.name, count(cr)) " +
            "from Credential cr join cr.client c group by c.name order by count(cr) desc")
